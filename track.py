...
class identity(object):
    ...
    colors_used = numpy.full(len(colors), False)
    max_dim = None
    ...
    def __init__(self):
        ...
        while identity.colors_used[colorid]:
            colorid = np.random.choice(len(identity.colors))
        self.color = identity.colors[colorid]
        identity.colors_used[colorid] = True
...
if __name__ ==  '__main__':
    ...
    for idx in range(offset, ttl_num):
        ...
        id_list = kalman_prediction(id_list, Q)     # todo
        ...
        if detect_flag:
            ...
            id_list, candidate_list = kalman_update(id_list, candidate_list, measured_positions, R)     # todo
